...
词法作用域是在写代码或者说定义时确定的，而动态作用域是在运行时确定的（this也是）（你不知道的js）。
...
事件循环的本质是一个“检查并执行”的循环过程。其严格的执行顺序如下：

1. **执行第一个宏任务**：通常是执行整段 `script` 脚本。
2. **清空微任务队列**：
   - 当一个宏任务（如同步代码）执行完毕后，JS 引擎会去检查微任务队列。
   - **重要：** 如果微任务队列中有任务，会**依次清空所有**微任务，直到队列为空。
   - 如果在执行微任务的过程中又产生了新的微任务，新产生的微任务也会在当前阶段被执行。
3. **（可选）更新渲染**：检查是否需要进行 UI 渲染。如果浏览器认为此时需要重绘或回流，会执行渲染逻辑。
4. **执行下一个宏任务**：从宏任务队列中取出一个（仅一个）任务放入调用栈执行。
5. **重复上述步骤**。
...
2. 没有prototype属性，也不能用作构造函数
3. 不能使用super、new.target
4. 没有arguments，可以使用rest参数获取（...args）
...
| sticky   | 它基本上是相对位置和固定位置的混合体，它允许被定位的元素表现得像相对定位一样，直到它滚动到某个阈值点（例如，从视口顶部起10像素）为止，此后它就变得固定了 |
...
| 属性名      |                      属性值                      |                             备注                             |
| ----------- | :----------------------------------------------: | :----------------------------------------------------------: |
| order       |                      [int]                       |      通过order属性改变，数值小的在前面，还可以是负数。       |
| flex-grow   |                     [number]                     | 扩展比率,根据弹性盒子元素所设置的扩展因子作为比率来分配剩余空间。<br />默认为0，不放大 |
| flex-shrink |                     [number]                     | 收缩比率,根据弹性盒子元素所设置的收缩因子作为比率来收缩空间。<br />默认为1，即如果空间不足，该项目将缩小。0是不缩小 |
| flex-basis  |                  [length], auto                  | 设置或检索弹性盒伸缩基准值。<br />默认值为`auto`，即项目的本来大小 |
| flex        |          上面三个的简写， 默认0 1 auto           | 默认:0 1 auto<br />`auto` :`1 1 auto` <br /> none :`0 0 auto`<br />1: 1 1 0% |
...
![img](https://raw.githubusercontent.com/pushmetop/resource/master/pushmetop.github.io/your-dont-know-css-center/mindmaps.png)
...
答案是不可以的。这是因为 JavaScript 引擎需要用栈来维护程序执行期间上下文的状态，如果栈空间大了话，所有的数据都存放在栈空间里面，那么会影响到上下文切换的效率，进而又影响到整个程序的执行效率。比如文中的 foo 函数执行结束了，JavaScript 引擎需要离开当前的执行上下文，只需要将指针下移到上个执行上下文的地址就可以了，foo 函数执行上下文栈区空间全部回收，具体过程你可以参考下图：![img](https://static001.geekbang.org/resource/image/d7/7b/d7153d003a72dbd0a9ca84b59ac3857b.png?wh=1142*532)
...
https://gemini.google.com/app/005f4b3ccb784187
...
1. 构建请求： 首先，浏览器构建**请求行**信息（如下所示），构建好后，浏览器准备发起网络请求。
...
3. 浏览器使用 **HTTP 协议作为应用层协议**，用来封装请求的文本信息；并使用 **TCP/IP 作传输层协议**将它发到网络上，所以在 HTTP 工作开始之前，浏览器需要通过 TCP 与服务器建立连接。也就是说 **HTTP 的内容是通过 TCP 的传输数据阶段来实现的**
...
[《05 | 渲染流程（上）：HTML、CSS 和 JavaScript 文件，是如何变成页面的？》](https://time.geekbang.org/column/article/118205)
...
构建渲染树（construct）：解析对应的CSS样式文件信息，计算出 DOM 树所有节点的样式

布局渲染树（reflow/layout）：从根节点递归调用，计算每一个元素的大小、位置等，给出每个节点所应该在屏幕上出现的精确坐标；

绘制渲染树（paint/repaint）：遍历渲染树，使用UI后端层来绘制每个节点。

![img](https://static001.geekbang.org/resource/image/63/4c/6391573a276c47a9a50ae0cbd2c5844c.jpg)css在加载过程中不会影响到DOM树的生成，但是会影响到Render树的生成，进而影响到layout，所以一般来说，style的link标签需要尽量放在head里面，因为在解析DOM树的时候是自上而下的，而css样式又是通过异步加载的，这样的话，解析DOM树下的body节点和加载css样式能尽可能的并行，加快Render树的生成的速度。
...
[硬核！30 张图解 HTTP 常见的面试题](https://mp.weixin.qq.com/s?__biz=MzUxODAzNDg4NQ==&mid=2247483971&idx=1&sn=8f2d5dae3d95efc446061b352c8e9961)
...
![img](https://p3.koolearn.com//005329buxuu0mg0mm00xr0.png)
...
[图解被问千百遍的 TCP 三次握手和四次挥手面试题](https://mp.weixin.qq.com/s/ni_zJpEQ-GvNjIZG6Cf4sw)
...
[几幅图，拿下 HTTPS](https://mp.weixin.qq.com/s?__biz=MzUxODAzNDg4NQ==&mid=2247487650&idx=1&sn=dfee83f6773a589c775ccd6f40491289&chksm=f98e5408cef9dd1ed900a15c27f00c811a5587ffa59a90a69a73d1794800838b6fd4b061ff9f&scene=178&cur_album_id=1337204681134751744#rd)
...
**![img](https://ugc-assets.km.tencent.com/asset/article/content/2512/3dd4b2eb44744694.jpeg?imageMogr2%2Fthumbnail%2F1540x%3E%2Fauto-orient%2Fignore-error%2F1&sign=1766386120-2kdSwdBaA9UJMn79-0-9f43f744d74499ae6dfbe4232b181ff4)**

MVC模式存在很多的变种，比如Apple MVC，Spring MVC，但是最传统的MVC模式如上图所示。

三层结构各自的说明如下：

**Model数据层**是应用程序的数据, 并且可以在数据变更时通知UI更新。

**View视图层**是用户看到的界面视图，从Model获取数据，接受用户的输入。

**Controller控制层**是执行用户的输入，通过调用Model实现相关业务逻辑，并可直接更新UI。

## 2. **MVP模式**

![img](https://ugc-assets.km.tencent.com/asset/article/content/2512/7e58b0bc927c474c.jpeg?imageMogr2%2Fthumbnail%2F1540x%3E%2Fauto-orient%2Fignore-error%2F1&sign=1766386144-NByecibl4688AlCM-0-07950e0f7df0fa57eb1222eb82c8074a)

MVP和MVC最大的区别，在于MVP中的View不再直接依赖Model，而是通过P(Presenter)去获取，减少了层级之间的依赖关系。而且Presenter是和View松耦合，可以实现Presenter业务的复用。

三层结构各自的说明如下：

**Model数据层**是应用程序的数据，不依赖其他层。

**View视图层**是用户看到的界面视图，通过Presenter从Model获取数据，接受用户的输入。

**Presenter展示层**是从Model获取数据提供给View层；并且执行用户的输入，通过调用&更新Model实现相关业务逻辑，然后再通过View接口间接的更新UI(只依赖接口，不依赖具体View的实现)。

## 3. **MVVM模式**

![img](https://ugc-assets.km.tencent.com/asset/article/content/2512/f4d1c05a9d644d0c.jpeg?imageMogr2%2Fthumbnail%2F1540x%3E%2Fauto-orient%2Fignore-error%2F1&sign=1766386158-67uC1BDwLONsoUoB-0-67195b72aa99379fae9ffbf348cae012)

MVVM和MVP模式从依赖关系上来来说是非常相似的。不过它们有一个最大的区别是MVVM是实现了View和ViewModel的自动同步机制，MVP在数据更新后，还需要我们手动调用View接口更新，但是MVVM可以实现自动更新，相当于MVP是手动挡，MVVM是自动挡。

MVVM避免了像MVP的View需要实现一大堆的UpdateXXX方法，简化了代码实现，但是也要注意统一的更新机制是否存在性能开销。

三层结构各自的说明如下：

**Model层**是应用程序的数据，也是独立的。

**View视图层**是用户看到的界面视图，和View Model进行绑定，监听View Model数据变更，收到变更后自动进行刷新。

**ViewModel层**主要负责转换Model数据为View所需要的格式、管理View的状态、处理业务逻辑。当View Model数据变更时自动向外发出变更通知。
...
进程 (Process)：资源的持有者。**操作系统分配资源（如内存、文件句柄）的最小单位**。当你打开一个浏览器、一个微信或者一个游戏时，系统就会为每一个应用创建一个或多个进程

线程 (Thread)：执行的实体。**CPU 调度和执行的最小单位****。它是进程内部的一个执行序列，一个进程可以包含多个线程，这些线程共享该进程的资源。
...
[深入理解 JavaScript 之手写浅拷贝和深拷贝](https://github.com/Jacky-Summer/personal-blog/blob/master/%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3JavaScript%E7%B3%BB%E5%88%97/%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3JavaScript%E4%B9%8B%E6%89%8B%E5%86%99%E6%B5%85%E6%8B%B7%E8%B4%9D%E5%92%8C%E6%B7%B1%E6%8B%B7%E8%B4%9D.md#%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3-javascript-%E4%B9%8B%E6%89%8B%E5%86%99%E6%B5%85%E6%8B%B7%E8%B4%9D%E5%92%8C%E6%B7%B1%E6%8B%B7%E8%B4%9D)
...
[2017\18\19面试记](https://q.shanyue.tech/interviews/2018.html)
...
- 前端错误监控上报
  - window.onerror
  - 过滤请求错误
  - Promsie then回调中错误会被catch吞掉
- 错误边界，不改变原有组件层次
  - 尝试try-catch父组件render方法：父组件render方法中，子组件并没有被实例化，父组件render中的try并不能捕获到子组件生命周期内的错误
  - 尝试错误边界：根组件添加错误边界 无法满足需求
  - 表格每一行，表单每一个表单项： 错误边界只能捕获子组件的渲染错误无法捕获组件自身错误
  - 错误上报耦合处理：throw error到全局
  - 触发错误后，打开其他表单弹窗state.error未还原，直接触发错误兜底render
- 自动化测试
...
2.[前端开发核心知识进阶：50 讲从夯实基础到突破瓶颈](https://www.zhihu.com/xen/market/remix/paid_column/1167078439772721152)
